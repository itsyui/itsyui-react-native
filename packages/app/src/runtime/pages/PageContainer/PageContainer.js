...
class PageContainer extends Component {
	...
	getToolbarWidgetsIds() {
		const { components } = this.props;
		const toolbarIds = Object.keys(components).filter(comp => components[comp].properties["ui:widget"] == "toolbar" || components[comp].properties["ui:widget"] == "itsy:toolbar");
		return toolbarIds;
	}

	getWidgets() {
		const { components, queryParams, currentPageQueryParams, designerMetadata, pagesPath } = this.props;
		const widgets = {};
	...
	getCell(cell, widgets, pageId, cellIdx, toolbarIds = null, isWrapWithView = true) {
		if (Array.isArray(toolbarIds) && toolbarIds.includes(cell.widget)) {
			return null;
		}
		if (cell.hasOwnProperty("widget")) {
			...
			const cellStyle = {
				...
				padding: cellProps && cellProps.properties.padding ? cellProps.properties.padding : 0,
			...
			return (
				<StateManagerContext.Provider key={`rt-provider-${cellIdx}`} value={{ contextPath: { ...contextPath, pageId } }}>
					{isWrapWithView ?
						<View style={[styles.widget_wrapper, { ...cellStyle }]}>
							{widgets[cell.widget]}
						</View>
						:
						widgets[cell.widget]
			...
			return this.getRows([cell], widgets, pageId, toolbarIds);
		}
	}

	getCells(columns, widgets, pageId, toolbarIds) {
		return columns.map((column, index) => {
			...
			if (Array.isArray(column.cell) && column.cell.length > 0) {
				cells = column.cell.map((x, i) => {
					return this.getCell(x, widgets, pageId, i, toolbarIds);
				});
			}
	...
	getRows(layoutRows, widgets, pageId, toolbarIds) {
		return layoutRows.map((item, index) => {
			...
			if (item && Array.isArray(item["columns"]) && item["columns"].length > 0) {
				...
				cells = this.getCells(columns, widgets, pageId, toolbarIds)
				...
				const col = <SchemaContainer schema={colSchema}>
					{this.getCell(item, widgets, pageId, Math.floor(Math.random() * 1000) + 1, toolbarIds)}
				</SchemaContainer>;
				cells.push(col);
			}
	...
	renderToolbar(toolbarIds, widgets, pageId) {
		if (Array.isArray(toolbarIds) && toolbarIds.length > 0) {
			return toolbarIds.map((id, cellIdx) => {
				return <>
					{this.getCell({ "widget": id }, widgets, pageId, cellIdx, null, false)}
				</>
			})
		}
		return null;
	...
	renderLayout() {
		...
		const toolbarIds = this.getToolbarWidgetsIds();
		//
		if (Array.isArray(layout) && layout.length > 0) {
			...
			return <SchemaContainer schema={gridSchema} >
				<ScrollView>
					{this.getRows(layout, widgets, pageId, toolbarIds)}
				</ScrollView>
				<>
					{this.renderToolbar(toolbarIds, widgets, pageId)}
				</>
			</SchemaContainer>
		}
