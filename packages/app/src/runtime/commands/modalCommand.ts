...
commandBindProvider.putSchema("modal", (metadata) => {
	const commandObj: CommandOptions<any> = {
		...
		execute: async (data: any, transition: (data: any) => void) => {
			...
			queryParams = controlSchema.properties["ui:widget"] === "r" || controlSchema.properties["ui:widget"] === "itsy:page" ? { ...getRuntimeQueryParams(controlSchema.properties["pageQueryParams"], { ...queryParams, ...commandQueryParams }, record) } : { ...rootPageContext.urlParams.queryParams, ...commandQueryParams };
			const pageId = rootPageContext && rootPageContext.currentPage ? rootPageContext.currentPage.id : null;
