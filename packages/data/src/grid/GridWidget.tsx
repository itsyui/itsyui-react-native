...
class GridWidget extends React.Component<GridWidgetProps, {}> {
	...
	getActions = (record) => {
		...
		const { _items, _typeId, contextPath } = (this.props as any);
		if (viewAttributes.customAction && Array.isArray(viewAttributes.customAction.actions)) {
			const refreshedItems = viewAttributes.customAction.actions.map(t => {
				...
				const data = { "data": record, "config": t };
				if (cmd) {
	...
	initializeGrid(schema = null) {
		const { typeId, gridSchemaId, gridSchema, filterText, idField, selectedRecords,
			rowSelectionMode, canTriggerGridSelectedRows, context, queryParams, dataSource, gridViewAttributes } = this.getControlSchemaProperties();
	...
	_getGridUIControlSchema() {
		const { typeId, gridViewType, gridViewAttributes, customGridViewId, emptyRecordsMessage, primaryColumn } = this.getControlSchemaProperties();
	...
	render() {
		if (!this.props.loaded) {
			return <Text>{getlocaleText("{{grid.init}}")}</Text>
		}
