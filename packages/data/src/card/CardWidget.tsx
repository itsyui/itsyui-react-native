...
class CardWidget extends React.Component<any, {}> {
	...
	componentWillMount() {
		const { title, secondaryTitle, cardType, items, titleAlign } = this.getControlSchemaProperties();
...
const mapDispatchToProps = (dispatch) => {
	return {
		onCardInit: (event) => dispatch(doCardInit(event.data)),
		onCardLoad: (event) => dispatch(doCardLoad(event.data)),
		onCardRefresh: (event) => dispatch(doCardRefresh(event.data)),
