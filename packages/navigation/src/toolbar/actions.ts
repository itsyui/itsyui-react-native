...
export function doToolbarRefresh(evt: any) {
	...
	return async (getState: any, dispatch: any, transition: any) => {
		...
		const contextPath = evt.params !== undefined && evt.params.contextPath !== undefined ? evt.params.contextPath : {};
		const context = evt.params !== undefined && evt.params.context !== undefined ? evt.params.context : {};
		// append typeId in the context always
		context.typeId = typeId;
		const cParams = evt.params !== undefined && evt.params.contextParams !== undefined ? evt.params.contextParams : contextParams;
