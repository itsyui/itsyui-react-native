...
class AuthContainer extends React.Component {
	...
	componentWillMount() {
		...
		const { showForgotPassword, showSignup, signupFormSchemaId, showLoadingIndicator, themeView, logo } = this.getControlSchemaProperties();
		const properties = {
	...
	handleLocationChange = (location) => {
		...
		if (routeMatches[path] !== this.props.page) {
			if (path === "login") {
				const { showForgotPassword, showSignup, signupFormSchemaId, logo } = this.getControlSchemaProperties();
				this.props.transition({
					type: TenantLoginActions.State.TENANT_AUTH_FORM_BEFORE_LOGIN_RENDER,
					showForgotPasswordButton: showForgotPassword,
					showSignupButton: showSignup,
